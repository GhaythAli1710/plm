...
class MrpProduction(models.Model):
    ...
    latest_bom_id = fields.Many2one(
        comodel_name='mrp.bom',
        compute="_compute_latest_bom_id",
    )
    ...
    def action_create_eco(self):
        ...
        eco_type = self.env.ref('plm.ecotype_bom_update', raise_if_not_found=False)
        if not eco_type:
            eco_type = self.env['mrp.eco.type'].sudo().search([], limit=1)
        eco_stage = eco_type.stage_ids[:1]
        action = self.env['ir.actions.act_window']._for_xml_id('plm.mrp_eco_action_main')
    ...
    def action_open_eco(self):
        ...
        if len(self.eco_ids) == 1:
            ...
            action['views'].insert(0, action['views'].pop(1))
        return action
