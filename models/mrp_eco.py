...
class MrpEcoType(models.Model):
    ...
    stage_ids = fields.Many2many(
        ...
        relation='mrp_eco_stage_type_rel',
        column1='type_id',
        column2='stage_id',
        string='Stages',
    )
...
class MrpEcoApprovalTemplate(models.Model):
    ...
    user_ids = fields.Many2many(
        ...
        string='Users',
        domain=lambda self: [('groups_id', 'in', self.env.ref('plm.group_plm_user').id)],
        required=True,
    )
...
class MrpEcoApproval(models.Model):
    ...
    approval_template_id = fields.Many2one(
        comodel_name='mrp.eco.approval.template',
        string='Template',
        ondelete='cascade',
        required=True,
    )
    name = fields.Char(
        string='Role',
        related='approval_template_id.name',
        store=True,
        readonly=False,
    )
    ...
    awaiting_my_validation = fields.Boolean(
        compute='_compute_awaiting_my_validation',
        search='_search_awaiting_my_validation',
    )
...
class MrpEcoStage(models.Model):
    ...
    legend_blocked = fields.Char(
        ...
        translate=True,
        required=True,
        help='Override the default value displayed for the blocked state for kanban selection, when the ECO is in '
             'that stage.',
    )
    legend_done = fields.Char(
        ...
        translate=True,
        required=True,
        help='Override the default value displayed for the done state for kanban selection, when the ECO is in that '
             'stage.',
    )
    legend_normal = fields.Char(
        ...
        translate=True,
        required=True,
        help='Override the default value displayed for the normal state for kanban selection, when the ECO is in that '
             'stage.',
    )
    description = fields.Text(
        help="Description and tooltips of the stage states.",
    )
...
class MrpEco(models.Model):
    ...
    allow_change_kanban_state = fields.Boolean(
        string='Allow Change Kanban State',
        compute='_compute_allow_change_kanban_state',
    )
    allow_change_stage = fields.Boolean(
        string='Allow Change Stage',
        compute='_compute_allow_change_stage',
    )
    allow_apply_change = fields.Boolean(
        string='Show Apply Change',
        compute='_compute_allow_apply_change',
    )
    ...
    new_bom_id = fields.Many2one(
        comodel_name='mrp.bom',
        string='New Bill of Materials',
        copy=False,
    )
    new_bom_revision = fields.Integer(
        string='BoM Revision',
        related='new_bom_id.version',
        store=True,
        readonly=False,
    )
    bom_change_ids = fields.One2many(
        comodel_name='mrp.eco.bom.change',
        inverse_name='eco_id',
    ...
    bom_rebase_ids = fields.One2many(
        comodel_name='mrp.eco.bom.change',
        inverse_name='rebase_id',
        string="BoM Rebase",
    )
    routing_change_ids = fields.One2many(
        comodel_name='mrp.eco.routing.change',
        inverse_name='eco_id',
    ...
    displayed_image_attachment_id = fields.Many2one(
        ...
        related='displayed_image_id.ir_attachment_id',
        readonly=False,
    )
    color = fields.Integer(
        string='Color',
    ...
    active = fields.Boolean(
        string='Active',
        default=True,
        help="If the active field is set to False, it will allow you to hide the engineering change order without "
             "removing it.",
    )
    ...
    def _is_conflict(self, new_bom_lines, changes=None):
        ...
        for reb_line in changes:
            ...
            if new_line and (reb_line.old_operation_id, reb_line.old_uom_id, reb_line.old_product_qty) != (
        ...
        if self.new_bom_id:
            for line in new_bom.bom_line_ids:
                ...
                if old_line and (line.product_uom_id != old_line.product_uom_id or
                                 tools.float_compare(line.product_qty, old_line.product_qty,
        ...
        for reb_line in rebase_lines:
            ...
            if new_bom_line:
                if new_bom_line.product_qty + reb_line.upd_product_qty > 0.0:
                    new_bom_line.write({'product_qty': new_bom_line.product_qty + reb_line.upd_product_qty,
    ...
    def apply_rebase(self):
        ...
        if self.current_bom_id:
            self.new_bom_id.write(
                {'version': self.current_bom_id.version + 1, 'previous_bom_id': self.current_bom_id.id})
            vals.update({'bom_id': self.current_bom_id.id, 'current_bom_id': False})
        self.message_post(body=_('Successfully Rebased!'))
        return self.write(vals)

    @api.depends('bom_id.bom_line_ids', 'new_bom_id.bom_line_ids', 'new_bom_id.bom_line_ids.product_qty',
                 'new_bom_id.bom_line_ids.product_uom_id', 'new_bom_id.bom_line_ids.operation_id')
    def _compute_bom_change_ids(self):
        for eco in self:
            eco.bom_change_ids = eco._get_difference_bom_lines(eco.bom_id, eco.new_bom_id)

    @api.depends('bom_id.bom_line_ids', 'current_bom_id.bom_line_ids', 'current_bom_id.bom_line_ids.product_qty',
                 'current_bom_id.bom_line_ids.product_uom_id', 'current_bom_id.bom_line_ids.operation_id')
    ...
    @api.depends('bom_id.operation_ids', 'bom_id.operation_ids.active', 'new_bom_id.operation_ids',
    ...
    def _compute_user_approval(self):
        for eco in self:
            is_required_approval = eco.stage_id.approval_template_ids.filtered(
                lambda x: x.approval_type in ('mandatory', 'optional') and self.env.user in x.user_ids)
            user_approvals = eco.approval_ids.filtered(
                lambda x: x.template_stage_id == eco.stage_id and x.user_id == self.env.user and not x.is_closed)
    ...
    def write(self, vals):
        ...
        if 'displayed_image_attachment_id' in vals:
            ...
            if vals['displayed_image_attachment_id']:
                doc = self.env['mrp.document'].search(
                    [('ir_attachment_id', '=', vals['displayed_image_attachment_id'])])
                if not doc:
        ...
        if message.message_type == 'comment' and message.author_id == self.env.user.partner_id:  # should use message_values to avoid a read
            for eco in self:
                for approval in eco.approval_ids.filtered(
                        lambda app: app.template_stage_id == eco.stage_id and app.status == 'none' and app.approval_template_id.approval_type == 'comment'):
        ...
        for eco in self:
            for approval_template in eco.stage_id.approval_template_ids:
                approval = eco.approval_ids.filtered(
                    lambda app: app.approval_template_id == approval_template and not app.is_closed)
                if not approval:
                    ...
                    for user in approval_template.user_ids:
                        activity_vals.append({
                            'activity_type_id': self.env.ref('plm.mail_activity_eco_approval').id,
                            'user_id': user.id,
                            'res_id': eco.id,
                            'res_model_id': self.env.ref('plm.model_mrp_eco').id,
                        })
        ...
        for eco in self:
            for approval_template in eco.stage_id.approval_template_ids.filtered(lambda a: self.env.user in a.user_ids):
                approvals = eco.approval_ids.filtered(
                    lambda x: x.approval_template_id == approval_template and not x.is_closed)
                none_approvals = approvals.filtered(lambda a: a.status == 'none')
                confirmed_approvals = approvals - none_approvals
                if none_approvals:
                    none_approvals.write(
                        {'status': status, 'user_id': self.env.uid, 'approval_date': fields.Datetime.now()})
    ...
    def action_see_attachments(self):
        ...
        attachment_view = self.env.ref('plm.mrp_document_file_view_kanban_plm')
        context = {
...
class MrpEcoBomChange(models.Model):
    ...
    eco_id = fields.Many2one(
        comodel_name='mrp.eco',
        string='Engineering Change',
        ondelete='cascade',
    )
    eco_rebase_id = fields.Many2one(
        comodel_name='mrp.eco',
        string='ECO Rebase',
        ondelete='cascade',
    )
    rebase_id = fields.Many2one(
        comodel_name='mrp.eco',
        string='Rebase',
        ondelete='cascade',
    )
    ...
    old_uom_id = fields.Many2one(
        ...
        string='Previous Product UoM',
    )
    new_uom_id = fields.Many2one(
        comodel_name='uom.uom',
        string='New Product UoM',
    )
    old_product_qty = fields.Float(
        string='Previous revision quantity',
        default=0,
    )
    new_product_qty = fields.Float(
        string='New revision quantity',
        default=0,
    )
    old_operation_id = fields.Many2one(
        comodel_name='mrp.routing.workcenter',
        string='Previous Consumed in Operation',
    )
    new_operation_id = fields.Many2one(
        comodel_name='mrp.routing.workcenter',
        string='New Consumed in Operation',
    )
    upd_product_qty = fields.Float(
        string='Quantity',
    ...
    def _compute_change(self):
        for rec in self:
            ...
            if (
                    rec.old_operation_id._get_comparison_values() != rec.new_operation_id._get_comparison_values()) and rec.change_type == 'update':
...
class MrpEcoRoutingChange(models.Model):
    ...
    old_time_cycle_manual = fields.Float(
        string='Old manual duration',
        default=0,
    )
    new_time_cycle_manual = fields.Float(
        string='New manual duration',
        default=0,
    )
    upd_time_cycle_manual = fields.Float(
        string='Manual Duration Change',
        compute='_compute_upd_time_cycle_manual',
        store=True,
    )
    operation_id = fields.Many2one(
        comodel_name='mrp.routing.workcenter',
        string='New or Previous Operation',
    )
